...
local function is_lua51(bytes)
    if #bytes < 12 then return false end
    local b = { bytes:byte(1, 6) }
    -- 1B 4C 75 61 = ESC 'L' 'u' 'a'
    if b[1] ~= 0x1B or b[2] ~= 0x4C or b[3] ~= 0x75 or b[4] ~= 0x61 then return false end
    if b[5] ~= 0x51 then return false end -- version 0x51 (Lua 5.1)
    return true
end

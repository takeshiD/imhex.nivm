...
M.list = function()
    local names = {}
    for _, d in ipairs(registry) do names[#names + 1] = d.name end
    return names
end
...
local function pick_decoder(path, bytes)
    ...
    local preferred = {}
    for _, d in ipairs(registry) do preferred[d.name] = d end
    for _, name in ipairs(cfg.decode.prefer or {}) do
        local d = preferred[name]
        if d and d.match(bytes, path) then return d end
    end
    -- Fallback: first matching
    for _, d in ipairs(registry) do
        if d.match(bytes, path) then return d end
    end
    return nil
end
